...
def get_r_ti16_config():
  ...
  config.classifier = 'seg'
  config.decoder_channels = (512, 256, 128, 64, 16)
  config.skip_channels = [0, 0, 0, 64, 0]
  config.pretrained_path = '../model/vit_checkpoint/imagenet21k/R_Ti_16-i21k-300ep-lr_0.001-aug_light0-wd_0.03-do_0.0-sd_0.0--cifar100-steps_10k-lr_0.001-res_224.npz'
...
def get_r26_s32_config():
  ...
  config.pretrained_path = '../model/vit_checkpoint/imagenet21k/R26+S_32.npz'
  config.patches.grid = (7, 7)
  config.n_skip = 4
  config.classifier = 'seg'
  config.decoder_channels = (512, 256, 128, 64, 16)
  config.skip_channels = [1024, 512, 256, 64, 16]
  return config
